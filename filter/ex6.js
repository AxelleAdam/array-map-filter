...
function filterOffensiveComments(comments, bannedWords) {
  return comments.filter(comment => !hasWordBanned(comment, bannedWords));
}

function hasWordBanned(comment, bannedWords) {
  for (let i = 0; i < bannedWords.length; i++) {
    if (comment.toLowerCase().split(" ").indexOf(bannedWords[i]) > -1) {
      return true;
    }
  }
  return false;
